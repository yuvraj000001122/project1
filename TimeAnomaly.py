...
class IsolationForestDetector(AnomalyDetector):
    ...
    def detect(self, data: pd.DataFrame) -> List[Tuple[int, float]]:
        ...
        Intuition: Anomalies are easier to isolate in tree structures
        Approach: Train isolation forest on features and score samples
        ...
        Returns:
            List of anomaly indices with confidence scores
        """
        feature_cols = [col for col in data.columns if col not in ['timestamp']]
        ...
        for idx, (pred, score) in enumerate(zip(predictions, anomaly_scores)):
            if pred == -1:  # Anomaly
                confidence = abs(score)  # Convert to positive confidence
                anomalies.append((idx, confidence))
...
class AnomalyDetectionEngine:
    ...
    def process_batch_data(self, file_path: str, sensor_id: str) -> List[AnomalyAlert]:
        ...
        try:
            ...
            alerts = []
            for idx, confidence, anomaly_type in anomalies:
                alert = asyncio.run(self._create_alert(
                    sensor_id, processed_data.iloc[idx], 
                    confidence, anomaly_type, processed_data
                ))
                if alert:
...
async def main():
    ...
    try:
        ...
        alerts = engine.process_batch_data('sample_data.csv', 'meter_001')
